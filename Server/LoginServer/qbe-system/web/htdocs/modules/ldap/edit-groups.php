...
function print_list()
{
	...
	uasort($classinfo,"classInfoSort");
	...
	for ($group=0; $group<$mygroupcount; $group++)
        {	$gn = substr(strstr($mygroups[$group],"-"),1);	?>

		<th align=right><?=$gn?><br />
	...
	?>
		...
		<td><a href="display-user?uid=<?=$uobj["uid"][0]?>"><?=$uobj["cn"][0]?></a></td>
		<?
		...
		{	// do a case-insensitive (note the "case") against the dn and the member
			...
			$dn = str_replace(" ","",strtolower($uobj["dn"]));
			if (!strcmp($member,$dn))
			{	echo "CHECKED ";
				break;
			}
		}?> name="group[<?=$mygroups[$group]?>][<?=$uobj["uid"][0]?>]">
		</td>
...
//		echo "cn=" . $group . ", ou=" . $class . ", ou=Classes, ou=People, o=htlwrn, c=at<br>";
	...
	if ($error)
	{
		sas_pcode('error','LDAP Error');
  	} else {
		sas_pcode('success','&Auml;nderungen gespeichert.');
		print_list();
	}
