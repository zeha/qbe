...
sas_varImport('prefill');
	...
	?>
		...
		<tr>
		<form action="<?=$qbe_report_templates['user-by-uid']?>" method=post>
	...
	<button type=submit>Suchen</button> </td>
		...
		<tr>
		<form action="<?=$qbe_report_templates['user-by-sn']?>" method=post>
		<input name="popup" value="<?=$qbe_popup?>" type=hidden />
		<?php if ($qbe_popup) { ?><input name="action" value="../../core/lookup-helper?" type=hidden><?php }?>
		<td> Nachname: </td>
		<td> 
	<input name="value" value="<?=$prefill?>" maxlength="30" size="15"/>
	<button type=submit>Suchen</button> </td>
		</form>
		</tr>
		
		<tr>
		<form action="<?=$qbe_report_templates['class']?>" method=post>
		<input name="popup" value="<?=$qbe_popup?>" type=hidden />
		<?php if ($qbe_popup) { ?><input name="action" value="../../core/lookup-helper?" type=hidden><?php }?>
		<td> Klasse: </td>
		<td>
