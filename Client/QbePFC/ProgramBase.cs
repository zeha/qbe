...
namespace ComReg
{
	public class	ProgramBase
	{
		...
		static private Option[] baseoptions = 
						 {
							new Option ("/silent",	"","\t\tPrevents the display of error messages.", false),
							new Option ("/quiet","", "\t\tPrevents the display of any messages.", false),
							new Option ("/?", "/help", "\t\tDisplay this usage message.", false)
						 };
		static protected Option[] options;

		static protected bool silent = false;
		static protected bool quiet = false;
		
		static public void PrintLogo()
		{
			Console.WriteLine("Microsoft (R) " + name + " - " + description + " " /*+ Version.VersionString*/);
			Console.WriteLine("Qbe Software Setup");
			Console.WriteLine("Copyright (C) Microsoft Corp. 2000-2002.  All rights reserved.");
			Console.WriteLine("Copyright (C) Christian Hofstaedtler 2000-2002.  All rights reserved.");
			Console.WriteLine("");
		}
		...
		protected static Setting[] GetSwitches(String []args)
		{
			...
			foreach( Setting s in reply)
			{
				...
				if (s.option != null)
				{
					if (CompareString(s.option.value, "/silent")==0)
						silent = true;
					if (CompareString(s.option.value, "/quiet")==0)
						quiet = true;
				}
			}
		...
		{
			if (!quiet)
			System.Windows.Forms.MessageBox.Show(name + " Error:\n" + msg,"Qbe Software Setup",System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Exclamation);
		}

		static public	void WriteErrorMsg(Exception e)
		{
			if (!quiet)
			System.Windows.Forms.MessageBox.Show(name + " Error:\n" + e.ToString(),"Qbe Software Setup",System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Exclamation);
		}

		static public   void WriteInfoMsg(String msg)
		{
			if ((!quiet) || (!silent))
			System.Windows.Forms.MessageBox.Show(name + " Information:\n" + msg,"Qbe Software Setup",System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information);	
		}
